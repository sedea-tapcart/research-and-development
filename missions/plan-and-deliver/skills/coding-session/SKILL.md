...
7. **Repo rules reconciliation** — When plan-anchored, run [Repo rules reconciliation (binding)](#repo-rules-reconciliation-binding) and pass [Repo rules reconciliation gate](#repo-rules-reconciliation-gate) before steps **8**–**9** or [Ship cut-point gate](#ship-cut-point-gate-approve-commit-local-test). Skip when `anchorType` is free-form or plan **§5** is `_None — no repo rule updates required for this PR._` only.
8. **Run all tests and checks** — Immediately after step **5** completes (implementation ready for developer review), run every locally executable **test and check** per [Post-implementation tests and checks](#post-implementation-tests-and-checks). Re-run after each implementation batch (including returns via **`more-changes`**). **Block** [Ship cut-point gate](#ship-cut-point-gate-approve-commit-local-test) until step **8** passes.
9. **Ship chain** — When implementation is ready for developer review, step **7** reconciliation passes (or is skipped), and step **8** passes (all runnable tests and checks exit **0**, or none were discoverable — state why), follow [Ship chain after implementation](#ship-chain-after-implementation-coding-session-lane) on **this same lane** ([Ship cut-point gate](#ship-cut-point-gate-approve-commit-local-test) — one modal for approve + commit + Local test when applicable → **`pre-pr-review`** → **`create-pr`** → merge → post-merge Staging test when applicable). **Do not** skip Local test or open a PR before that order completes.

## Post-implementation tests and checks

**When:** After step **5** (implement) completes and **before** [Ship cut-point gate](#ship-cut-point-gate-approve-commit-local-test). Re-run after each code-change batch when the developer returns via **`more-changes`** or after pre-PR review fixes.
...
**Forbidden:** Opening [Ship cut-point gate](#ship-cut-point-gate-approve-commit-local-test) while tests or checks are failing; skipping runnable suites because implementation chat says *done*; treating developer *ready for review* as a substitute for running tests and checks.
...
| 8 | [Post-merge workspace cleanup](#post-merge-workspace-cleanup) | procedure | **No** — after **`prState: merged`**, before Staging / Production | **No** — auto **`--apply`** when authorized; modal on failure/unclear ownership only |
...
| 10 | [Production deploy-walk handoff](#production-deploy-walk-handoff) | inline | **No** — after Staging complete/skipped/empty | **No** (manual §7 step only) |
| 11 | [Plan-reconcile handoff (inline)](#plan-reconcile-handoff-inline) | inline | **No** — explicit start; not auto from deploy-walk | **Yes** when reconcile inventory requires picks; [Post–Production remainder authorization](#post-production-remainder-authorization) may batch tail work first |
...
| **`commit-only-skip-local-test`** (legacy **`commit-only-skip-local-test`**) | 1. **`git commit`** if dirty · 2. Append dated skip note under §7 or **`## Follow-ups`** · 3. [Auto-spawn pre-pr-review](#auto-spawn-pre-pr-review) |
...
| **`spawn-local-test-walk`** (legacy **`spawn-local-test-walk`**) | [Local test deploy-walk handoff](#local-test-deploy-walk-handoff) inline |
| **`skip-local-test`** (legacy **`skip-local-test`**) | Dated skip note · [Auto-spawn pre-pr-review](#auto-spawn-pre-pr-review) |
...
1. **Read** §7 **`### Local test`** (legacy **`### Local test`** reads as Local test). If empty, only *None — …*, or every item is `[x]`, note in one line and continue to [Auto-spawn pre-pr-review](#auto-spawn-pre-pr-review).
...
Run **post-merge** after [Post-merge workspace cleanup](#post-merge-workspace-cleanup) completes or is skipped — see [Post-merge deploy routing](#post-merge-deploy-routing). **Normative timing:** merged code is on **`origin/main`**, staging environment deploys the merged commit, then Staging §7 steps run before Production.

1. **Precondition:** `prState: merged`; plan §7 **`**Status:**`** is **`deployed`** (run **`deploy-walk deployed`** when still `pr-open`).
2. **Read** §7 **`### Staging test`**. If empty, only *None — …*, or every item is `[x]`, note in one line and continue to [Production deploy-walk handoff](#production-deploy-walk-handoff) or [Plan-reconcile handoff (inline)](#plan-reconcile-handoff-inline).
...
5. When `stagingTestStatus` is `complete` or documented skip, continue to [Production deploy-walk handoff](#production-deploy-walk-handoff) when §7 **`### Production`** applies.
6. **Do not** run Production inline walk in this scope — Staging test is post-merge only (before Production).
...
| **`spawn-production-walk`** | When merge confirmed and Staging complete/skipped/empty: [Post-merge workspace cleanup](#post-merge-workspace-cleanup) **auto-apply** on **next** turn, then [Production deploy-walk handoff](#production-deploy-walk-handoff) |
...
3. When Staging is empty, all `[x]`, or documented skip → [Production deploy-walk handoff](#production-deploy-walk-handoff) when §7 **`### Production`** applies per hosting profile; otherwise close deploy lifecycle per dashboard **staging-only** profile.
4. **Forbidden:** [Production deploy-walk handoff](#production-deploy-walk-handoff) while Staging `[ ]` remain without documented skip.

At [Post-create-pr handoff gate](#post-create-pr-handoff-gate) when `prState: merged`, prefer **`spawn-staging-test-walk`** when Staging `[ ]` remain; use **`spawn-production-walk`** only when Staging is complete/skipped/empty.
