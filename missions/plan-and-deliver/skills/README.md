...
| `deploy-walk` | Active **`coding-session`** agent on its lane (Local test after commit, Production after merge, or deploy phrases) | `## Completion (inline)` | `deploy-walk` via **`coding-session`** terminal re-emit — no separate child terminal |
