...
| --- | --- | --- |
| `ad-hoc-prd` | `.sedea/centers/research-and-development/missions/plan-and-deliver/skills/ad-hoc-prd/SKILL.md` | Scaffold a minimal **Ad-Hoc PRD** (`ad_hoc_<slug>_<hex>.ad-hoc-prd.md`) under **`.sedea/operations/<operations-user-id>/docs/`** only — standalone upstream input for **`master-plan`**; Master Plan `.plan.md` is created afterward. Moving an Ad-Hoc PRD into **`joint/docs/`** for sharing is manual. |
| `master-plan` | `.sedea/centers/research-and-development/missions/plan-and-deliver/skills/master-plan/SKILL.md` | PRD → **Master Plan** (mode #1). Drafts §§ 1–5 of the Master Plan template (Background, Benefits, Related features, Architectural design, Changes) in the initial turn, including **`### Decomposition assessment`** and **`### Complexity score (plan-scope signal)`** under § 5. The complexity subsection is a **fixed two-column, three-row table** (flowchart boxes §4, sequence arrows §4, change bullets §5) plus **band** and **overall score** (max of the three rows); **high** (overall score > 20) gates foregrounding **`6`** until scope shrinks or the feature is split by user journeys. § 6 routes out to `delivery-phases` or `pr-breakdown`; § 7 (Caveats) is drafted by this protocol branch in a follow-up turn via the `7` shortcut. |
...
| `new-plan` | `.sedea/centers/research-and-development/missions/plan-and-deliver/skills/new-plan/SKILL.md` | Scaffold a new `.plan.md` + sidecar; parent linkage; invoked by **`sub-plan`** to spawn child plans from `Delivery phases` / `PR breakdown` numbered lists. |
...
- **Sedea rules impact** (mode #3 per-PR § 5) — short bullets for **`.cursor/rules/*.mdc`** in the hosting product repo (what to update with the code) and for **`.sedea/centers/research-and-development/rules/maintain-rules.mdc`** (center rule hygiene); agent-primary. The `_None — …_` line is still short-bullet form.
...
4. **A new sub-plan** spawned via **`sub-plan`** (phase of a parent plan resolved from chat context per **`.sedea/centers/research-and-development/rules/planning-target-resolution.mdc`**) or **`new plan`** (free-standing).
